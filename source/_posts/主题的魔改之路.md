...
```
careers.css:
新建：`\themes\butterfly\source\_custom\about`中建`careers.css`
```
.author-content-item.careers {
    min-height: 400px;
  }
...
新建：`\themes\butterfly\source\_custom\about`中新建`genshinimpact.css`
```
:root {
...
hello-about.css：
新建：`\themes\butterfly\source\_custom\about`中新建`hello-about.css`
```
.hello-about {
    margin: 20px auto;
    border-radius: 24px;
...
skills.css:
新建：`\themes\butterfly\source\_custom\about`新建`skills.css`
```
.author-content-item.skills {
    display: flex;
    justify-content: center;
...
site.css：
新建：`\themes\butterfly\source\_custom\about`中新建`site.css`
```
...
修改颜色代码：
若要颜色正常显示，将所有例如`var(--HiPeach-yellow)`的颜色变量代码修改成`#FFFFFF`或者`rgb(255, 255,255)`的色码
...
添加赞赏数据：
新建：`\source\_data`中新建`reward.yml`
e.g.:
```
- class_name: 赞赏
  reward_list:
...
添加创造力数据：
新建：`source/_data`中新建`creativity.yml`
e.g.：
```
- class_name: 开启创造力
  creativity_list:
...
在根目录下执行：`npm install sweetalert2 --save`，这是  插件
